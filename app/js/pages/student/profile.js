...
export async function render(container) {
    ...
    const details = [
        ...
        { label: 'Year Level', value: user.year_level ? `Year ${user.year_level}` : '—' },
        { label: 'Joined',     value: fmtDate(user.created_at) },
    ];

    const statsHtml = `
        <div class="p-stat-row">
            <div class="p-stat"><span class="p-stat-num">${stats.subjects || 0}</span><span class="p-stat-lbl">Subjects</span></div>
            <div class="p-stat"><span class="p-stat-num">${stats.lessons_completed || 0}</span><span class="p-stat-lbl">Lessons</span></div>
            <div class="p-stat"><span class="p-stat-num">${stats.total_quizzes || 0}</span><span class="p-stat-lbl">Quizzes</span></div>
        </div>`;

    container.innerHTML = sharedStyles() + `
        <style>
            .p-stat-row { display:grid; grid-template-columns:repeat(3,1fr); border-top:1px solid #f3f4f6; }
            .p-stat { text-align:center; padding:14px 8px; }
            .p-stat:not(:last-child) { border-right:1px solid #f3f4f6; }
            .p-stat-num { display:block; font-size:20px; font-weight:800; color:#00461B; }
            .p-stat-lbl { font-size:10.5px; color:#9ca3af; text-transform:uppercase; letter-spacing:.5px; }
        </style>
        <div class="p-layout">
...
function mkAlert(msg, type) { return `<div class="p-alert p-alert-${type}" style="margin-bottom:14px;">${esc(msg)}</div>`; }
