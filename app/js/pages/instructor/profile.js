...
export async function render(container) {
    ...
    renderShell(container, user, [
        { label: 'Employee ID', value: user.employee_id || '—', color: '#7c3aed' },
        { label: 'Email',       value: user.email,              color: '#0369a1' },
        { label: 'Department',  value: user.department_name || '—', color: '#0f766e' },
        { label: 'Program',     value: user.program_code ? `${user.program_code} — ${user.program_name}` : (user.program_name || '—'), color: '#b45309' },
        { label: 'Joined',      value: fmtDate(user.created_at), color: '#525252' },
    ]);
    bindSave(container, user);
...
function renderShell(container, user, details) {
    container.innerHTML = profileStyles() + `
        <div class="p-layout">
...
function bindSave(container, user) {
    container.querySelector('#btn-info').addEventListener('click', async () => {
        ...
        if (!payload.first_name || !payload.last_name || !payload.email) {
            al.innerHTML = alert('All fields are required.', 'err'); return;
        }
        const res = await Api.post('/AuthAPI.php?action=update-profile', payload);
        al.innerHTML = res.success
            ? alert('Profile updated successfully!', 'ok')
            : alert(res.message || 'Failed to update profile.', 'err');
...
function profileStyles() {
    return `<style>
        ...
        .p-input {
            ...
            box-sizing:border-box; width:100%; transition:border .15s, box-shadow .15s;
        }
